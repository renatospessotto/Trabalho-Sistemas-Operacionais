...
// Símbolos para as frutas em cada etapa
...
void desenhar_interface_completa() {
    ...
    mvprintw(0, 25, "=== FÁBRICA DE SUCOS ===");
    attroff(COLOR_PAIR(1) | A_BOLD);
    
    // Instruções de controle
    attron(COLOR_PAIR(3));
    mvprintw(1, 5, "Controles: [+/-] Ajustar máquinas | [1-4] Selecionar etapa | [q] Sair");
    ...
    for (int i = 0; i < 4; i++) {
        ...
        attron(COLOR_PAIR(3));
        mvprintw(linha_etapa + 1, 4, "Máquinas: %d", status_interface.num_maquinas[i]);
        attroff(COLOR_PAIR(3));
        ...
        attron(COLOR_PAIR(2));
        for (int j = 0; j < status_interface.frutas_em_etapa[i]; j++) {
            mvprintw(linha_etapa + 2, 17 + (j * 3), "%s", emojis_etapas[i]);
        }
    ...
    attron(COLOR_PAIR(1) | A_BOLD);
    mvprintw(linha_stats, 2, "ESTATÍSTICAS");
    attroff(COLOR_PAIR(1) | A_BOLD);
    
    attron(COLOR_PAIR(3));
    mvprintw(linha_stats + 1, 4, "Frutas concluídas: %d/%d", 
...
void marcar_fruta_entrando_etapa(int fruta_id, Etapa etapa) {
    ...
    for (int j = 0; j < frutas_processando; j++) {
        mvprintw(linha_etapa + 2, 17 + (j * 3), "%s", emojis_etapas[etapa]);
    }
    attroff(COLOR_PAIR(2));
...
void fruta_concluida(Fruta* f) {
    ...
    attron(COLOR_PAIR(1) | A_BOLD);
    mvprintw(linha_stats + 4, 4, "🎉 Fruta %d concluída!", f->id);
    attroff(COLOR_PAIR(1) | A_BOLD);
...
void atualizar_maquinas_etapa(Etapa etapa, int delta) {
    ...
    int linha_etapa = 4 + (etapa * 6);
    attron(COLOR_PAIR(3));
    mvprintw(linha_etapa + 1, 4, "Máquinas: %d  ", status_interface.num_maquinas[etapa]);
    attroff(COLOR_PAIR(3));
